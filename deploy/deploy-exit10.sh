...
BLP=$(forge create "$SRC/BaseToken.sol:BaseToken" --unlocked --constructor-args "Boost Liquidity" "BLP" | extract_addr)
echo "BLP: $BLP"
EXIT=$(forge create "$SRC/BaseToken.sol:BaseToken" --unlocked --constructor-args "Exit Liquidity" "EXIT" | extract_addr)

# Masterchefs
MASTERCHEF=$(forge create "$SRC/Masterchef.sol:Masterchef" --unlocked --constructor-args "$WETH" "$REWARDS_DURATION" | extract_addr)
...
EXIT10_DEPLOY_PARAMS="($NFT,$STO,$BOOT,$BLP,$EXIT,$MASTERCHEF_EXIT,$FEE_SPLITTER,$BENEFICIARY,$LIDO,$BOOTSTRAP_DURATION,$BOOTSTRAP_CAP,$LIQUIDITY_PER_USDC,$ACCRUAL_PARAMETER)"
