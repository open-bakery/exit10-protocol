...
contract Exit10_createBondTest is Exit10Test {
  ...
  function test_createBond() public {
    ...
    assertEq(__liquidity(), liquidityAdded, 'Liquidity added returned');
    _checkBondData(bondId, __liquidity(), 0, block.timestamp, 0, uint8(Exit10.BondStatus.active));
    assertGt(__liquidity(), 0, 'Check liquidity');
  ...
  function test_createBond_WithBootstrap() public {
    ...
    _checkBondData(bondId, __liquidity() - liquidityAdded, 0, block.timestamp, 0, uint8(Exit10.BondStatus.active));
    assertGt(__liquidity(), 0, 'Check liquidity');
    assertEq(nft.ownerOf(bondId), address(this), 'Check NFT owner');
    _checkBalancesExit10(0, 0);
